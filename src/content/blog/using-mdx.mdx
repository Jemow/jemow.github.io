...
j'ai eu pour mission de construire une scène 3D en utilisant C++ et OpenGL ES 3.0. 
...
sur les concepts importants plutôt que sur les détails d'implémentation.
...
Tout commence par l'importation de la géométrie. J'ai opté pour des models 
format standard Wavefront (.obj) pour leur simplicité.
...
```
Une fois le fichier lu, la fonction récursive ProcessNode parcourt la 
...
Pour simplifier cela, j'utilise la méthode SetTexture de l'abstraction Pipeline.
...
```
Dans la méthode Bind de la class Material, j'utilise cette abstraction pour lier toutes les maps
...
Le Normal Mapping sert à simuler du relief sur la carrosserie des voitures.
Le souci, c'est que les normales d'une "Normal Map" sont dans l'espace tangent (local à la face).
Pour les utiliser dans l'espace monde pour la lumière, il faut une matrice de transition : 
la matrice TBN.
...
```cpp
  ...
  vec3 B = cross(N, T);
```
J'utilise ici le procédé de Gram-Schmidt pour être sûr que la tangente reste parfaitement
orthogonale à la normale. Ensuite, je calcule la bitangente avec un Cross.
Ce calcul me permet de passer de l'espace tangent à l'espace monde (World Space), 
là où je fais tous mes calculs de lumière.
...
![Frsutum Camera example](/assets/frustum_example.png)
...
d'afficher des centaines d'objets identiques, comme mes cônes de chantier,
en un seul draw call. Au lieu d'envoyer chaque cône un par un,
...
Une matrice mat4 occupant 4 slots consécutifs (locations 5 à 8), j'utilise glVertexAttribDivisor.
Cela indique à OpenGL de ne passer à la matrice suivante qu'après avoir dessiné un objet complet, 
et pas après chaque sommet.
```cpp
...
```
Côté Shader, la logique est très simple. Si l'instancing est activé, j'utilise la matrice 
d'instance reçue en attribut (aInstanceMatrix) à la place de la matrice de modèle classique.
Cela permet de positionner chaque cône à sa coordonnée unique tout en utilisant la même géométrie.
```cpp
...
répercuter cet appel sur tous ses sub meshes.

Pour valider que l'instancing fonctionne correctement, j'ai utilisé RenderDoc afin d'inspecter les appels GPU.
Sur la capture ci-dessous, on peut voir l'appel glDrawElementsInstanced. Au lieu de dessiner 
un seul cône, le GPU en génère 4 en un seul passage en utilisant les données de mon instance 
buffer (évidemment on peut en instancier bien plus que 4 😅).
![Instancing RenderDoc example](/assets/instancing_renderdoc.png)
C'est cette technique qui permet de garder un framerate élevé même avec une scène chargée d'objets répétitifs.

## Cubemap

Avant d'attaquer le gros du moteur, il me fallait un environnement pour placer ma voiture. 
J'ai implémenté une Skybox. Le principe est simple : on dessine un cube géant autour de la caméra, 
et on applique une texture sur chaque face.

En OpenGL, on utilise une GL_TEXTURE_CUBE_MAP. C'est un type de texture spécial qui gère 6 faces
(Right, Left, Top, Bottom, Back, Front) comme une seule entité.
![Cubemap example](/assets/cubemap.png)
Pour le chargement dans l'abstraction common::Texture, l'astuce consiste à boucler sur les fichiers et à utiliser 
l'enum GL_TEXTURE_CUBE_MAP_POSITIVE_X. Comme les enums des 6 faces se suivent en mémoire dans OpenGL,
on peut simplement incrémenter i pour cibler la face suivante.
```cpp
...
- On change la fonction de profondeur à GL_LEQUAL pour être sûr que le ciel s'affiche bien au fond.
```cpp
...
Le Deferred Rendering est le cœur de mon pipeline de rendu. Contrairement au rendu classique où l'on calcule la lumière 
pour chaque objet au moment de le dessiner, le Deferred repose sur une séparation claire entre la 
récupération des données géométriques et le calcul final de l'image. Cette architecture m'a permis 
d'intégrer efficacement de nombreuses passes de rendu (Shadows, SSAO, Bloom) sans surcharger le GPU.
![Deferred Overview](/assets/deferred_overview.png)
Le but est simple : arrêter de calculer la lumière pour rien. Dans un rendu classique,
on perd beaucoup de performances à éclairer des objets qui finissent cachés derrière d'autres. 
Avec le G-Buffer (Geometry Buffer), on inverse la logique : on attend de savoir exactement quels 
pixels sont visibles à l'écran, et on ne calcule l'éclairage que pour eux.

## Geometry Buffer

Le G-Buffer est un ensemble de textures dans lesquelles on stocke les informations géométriques de la scène lors de la "Geometry Pass".
Au lieu de dessiner une image finale, on remplit plusieurs buffers simultanément :
- Positions
- Normales
- Albedo (BaseColor)
- Emissive

Pour que cela fonctionne techniquement, j'ai créé une classe GBuffer. L'élément le plus important
...
```
Une autre méthode cruciale est BlitDepthToDefault. Comme le Deferred Rendering s'effectue sur un simple 
Quad à la fin, on perdrait normalement les informations de profondeur. Cette fonction me permet de 
copier le buffer de profondeur du G-Buffer vers le buffer par défaut du système. 
Cela évite que la Skybox ou d'autres objets couvrent les voitures.
```cpp
...
```
Voici le rendu de mes différents buffers tel qu'on peut les observer lors d'une capture sur RenderDoc :
- Positions : Chaque pixel stocke ses coordonnées mondiales (XYZ).
...
- Emissive : Uniquement les zones qui émettent de la lumière.
![GBuffer Emissive](/assets/gbuffer_emissive.png)

## Shadow Pass

Avant d'attaquer le G-Buffer, je dois générer une Shadow Map pour calculer les ombres de ma lumière directionnelle.
Le principe est de faire une passe de rendu du point de vue de la lumière pour stocker
la profondeur de la scène dans une texture.

Dans ma classe ShadowMap, j'utilise un FBO avec uniquement un GL_DEPTH_ATTACHMENT.
Comme je n'ai pas besoin de couleurs ici, je désactive explicitement les buffers 
...
glBindFramebuffer(GL_FRAMEBUFFER, 0);
```
Pour une lumière directionnelle, j'utilise une projection orthographique. 
La matrice lightSpaceMatrix combine cette projection avec une vue positionnée face à la scène.
```cpp
...
```
Le shader est minimal. Il transforme les sommets directement dans l'espace de la lumière. Pas besoin de fragment shader complexe,
OpenGL gère l'écriture de la profondeur automatiquement.
```cpp
void main() {
    mat4 finalModel = useInstancing ? aInstanceMatrix : model;
...
```
Voici à quoi ressemble cette texture de profondeur capturée depuis la source lumineuse
![Shadow Map](/assets/shadow_map.png)
Une fois la texture de profondeur générée, je fais le calcul d'ombre dans mon fragment
shader principal, deferred_lit.frag. Pour éviter le Shadow Acne (des bandes noires dues
à l'imprécision du calcul), j'applique un Bias classique et un Normal Bias qui dépend
de l'inclinaison de la face par rapport à la lumière.
```cpp
...
```
Sur cette image, l'intensité de la lumière a été poussée pour bien voir les ombres. Sans aucun filtrage, 
les bords s'arrêtent net et l'aspect est très pixelisé
![Shadow without PCF](/assets/shadow.png)
Pour corriger ça, j'utilise le PCF. Le principe est de faire une "photo" des ombres
autour du pixel actuel en échantillonnant une grille 3x3 dans la shadow map,
puis de faire la moyenne. Cela crée un dégradé qui adoucit les contours.
![Shadow with PCF](/assets/shadow_pcf.png)
Même avec le PCF, on voit encore parfois une forme de grille dans l'ombre. Pour corriger ça, j'utilise un Poisson Disk.
Au lieu de tester les pixels sur une grille carrée toute droite, j'utilise 16 points placés de 
façon plus "aléatoire" mais bien répartis. Je me suis basé sur ce [LearnOpenGL](https://www.opengl-tutorial.org/intermediate-tutorials/tutorial-16-shadow-mapping/#poisson-sampling) pour l'implémentation.
```cpp
...
```
En multipliant une valeur aléatoire par 2π (6.28318530718), je génère un angle de rotation aléatoire.
Cet angle sert à faire pivoter les 16 points du disque différemment pour chaque pixel.
Cela permet de "mélanger" les samples. La fonction rand vient de cette [discussion Khronos](https://community.khronos.org/t/random-values/75728 ).
```cpp
float shadow = 0.0;
  vec2 texelSize = 1.0 / vec2(textureSize(shadowMap, 0));
  ...
  return shadow / 16.0;
```
Voici le résultat final. L'ombre est maintenant beaucoup plus "smooth" et naturelle grâce au
mélange du Poisson Disk et de la rotation aléatoire.
![Shadow with Poisson Sampling + Rotation](/assets/shadow_poisson.png)

## SSAO

Pour ajouter de la profondeur à la scène, j'ai appris comment implémenté le SSAO (Screen Space Ambient Occlusion).
Cette technique simule l'occultation de la lumière ambiante par la géométrie environnante.
Elle crée des ombres de contact dans les coins, les creux, et surtout sous la voiture,
ce qui permet de bien "ancrer" le modèle au sol. Voici un exemple si dessous :
![SSAO example](/assets/ssao_example.png)
Pour simplifier l'implémentation, j'ai créé une classe SSAO dédiée. Le rendu se fait en deux étapes : 
une passe de calcul (qui génère l'occlusion brute) et une passe de flou (pour nettoyer le bruit).

Je configure la première étape dans SSAO::Init, qui se charge d'appeler GenerateKernel et GenerateNoiseTexture.
Le calcul repose sur un échantillonnage statistique. Plutôt que de tirer des rayons partout, je génère un Kernel 
(hémisphère) de 64 échantillons orientés vers l'axe Z.
```cpp
...
```
Pour la texture de bruit de 4x4 pixels, je la génère ici procéduralement, mais on pourrait très
bien utiliser une simple image de bruit chargée depuis le disque. La texture est en GL_REPEAT
pour couvrir l'écran.
```cpp
std::uniform_real_distribution<float> random_floats(0.0, 1.0);
std::default_random_engine generator;
...
```
Une fois le Kernel et le bruit prêts, on passe au rendu.
![SSAO Result 1](/assets/ssao_pass1.png)
Mais souvenez-vous, le travail n'est pas fini. L'image brute est inexploitable à cause du bruit.
Il nous reste la 2ème passe : le Flou (Blur).

Dans la fonction de rendu, on voit bien l'enchaînement. Après avoir généré l'occlusion, 
...
```
Le shader de flou est très simple. Contrairement à un flou Gaussien (qu'on verra plus tard), 
...
```
Et voilà le résultat : le bruit a disparu pour laisser place à une occlusion douce et naturelle!
![SSAO Result 2](/assets/ssao_pass2.png)

## Light PBR

Une fois les données du G-Buffer et du SSAO prêtes, on peut enfin calculer l'éclairage. L'une des étape
à la fois cool niveau rendu et un peu plus complex a comprendre hehe...

Au départ, j'ai implémenté le modèle Blinn-Phong. C'est l'approche "classique" et peu coûteuse : 
on additionne une composante Diffuse (la couleur de l'objet) et une composante Spéculaire
(le reflet brillant). 
![Halfway example](/assets/halfway.png)
Le Half-way Vector (un vecteur médian entre la direction de la vue et celle
de la lumière) permet d'avoir le reflet spéculaire beaucoup plus vite 
qu'un calcul de réflexion physique complet.
![Blinn-Phong example](/assets/blinn_phong.png)
C'est deja pas mal, mais pyhsiquement parlant ce n'est pas très réaliste, afin d'avoir des voiture beaucoup
plus crédible je suis passé au PBR (Physically Based Rendering) qui est une méthode de rendu plus modern.

Pour obtenir ce réalisme, le PBR simule la surface des objets au niveau microscopique (Microfacets).
Même si une surface parait lisse, au microscope, elle est irrégulière.
Plus elle est "rugueuse", plus la lumière se disperse de manière chaotique.

Pour piloter cela, on abandonne les vieilles textures "Specular Map" pour
3 de textures physiques :

- Albedo : La couleur de base
- Metallic : Est-ce du métal ou du plastique/bois ? (0.0 ou 1.0).
- Roughness : L'état de surface (Lisse ou Rugueux).

Note d'optimisation : J'ai appris que dans l'industrie, on combine souvent l'Ambient Occlusion,
la Roughness et la Metallic dans une seule texture appelée "ARM" (Red, Green, Blue)
pour économiser de la mémoire.

De mon côté, je les ai simplement stockées dans les canaux Alpha (.a) du G-Buffer :
```cpp
...
Avec le PBR, Tout repose sur l'équation de réflectance. C'est la formule mathématique qui somme toute 
la lumière qui arrive pour calculer ce qui renvoie vers l'œil.
![Reflectance Equation](/assets/reflectance_equation.png)
Le terme le plus important dans cette équation est le BRDF (fr​). 
C'est lui qui décide comment la lumière réagit au contact de la surface. Dans mon moteur,
j'utilise le standard de l'industrie : le Cook-Torrance BRDF.

Ce modèle divise la lumière en deux parties distinctes :
- La Diffuse (Lambert) : La partie "couleur" (réfraction interne).
- La Spéculaire (Cook-Torrance) : La partie "reflet" (réflexion de surface).

L'équation complète du BRDF ressemble à ça :
![Cooke-Torrance BRDF](/assets/cook_torrance_brdf.png)
Pour la partie Diffuse, c'est très simple, c'est juste la couleur divisée par PI : c/π.
Pour la partie Spéculaire, c'est plus complexe. Elle utilise 3 fonctions physiques 
(Distribution, Fresnel, Geometry) pour simuler les microfacettes.
Je ne vais pas détailler les maths ici, mais pour les curieux, tout est expliqué en détail sur
[LearnOpenGL/PBR/Theory](https://learnopengl.com/PBR/Theory)
![Cook-Torrance Specular](/assets/specular.png)
Au final, quand on remplace fr​ dans l'équation de base, on obtient la formule complète que mon
shader résout pour chaque lumière :
![Reflectance Equation with Cook-Torrance](/assets/reflectance_equation_cook_torrance.png)
Pour traduire ces maths en code, on retrouve exactement la structure de l'équation dans la
boucle de lumière. On calcule les termes D, F, G pour le spéculaire, et on respecte la
conservation de l'énergie en ajustant la diffuse (kD) en fonction du spéculaire (kS).

Voici l'extrait du Fragment Shader qui gère une lumière :
...
Ok, avec ça, on a la base du PBR. Mais il manque un truc essentiel pour que ça claque vraiment.
Pour l'instant, notre Diffuse se résume juste à c/π, et le métal ne reflète rien du tout.
